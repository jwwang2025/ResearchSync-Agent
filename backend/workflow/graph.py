        ...
        for output in self.graph.stream(initial_state, config=config):
            ...
            if "__interrupt__" in output and not approval_handled:
                ...
                if isinstance(current_state, dict) and current_state.get('research_plan'):
                    ...
                    # 情况 C：既没有自动批准也没有回调，且未被外部批准 -> 让外部处理审批
                    else:
                        # 设置状态为等待审批，然后yield包含中断信息的输出
                        current_state['current_step'] = 'awaiting_approval'
                        try:
                            self.graph.update_state(config, current_state)
                        except Exception as e:
                            print(f"[stream_interactive] Failed to update state to awaiting_approval: {e}")
                        print(f"[stream_interactive] Workflow interrupted, awaiting external approval")
                        # 不要标记approval_handled，让外部代码处理审批
                        # yield当前的输出，让外部知道发生了中断
                        yield output
                        return  # 退出，让外部处理审批
